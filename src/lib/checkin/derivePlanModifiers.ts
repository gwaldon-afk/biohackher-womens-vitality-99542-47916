...
import type { NormalizedDailyCheckin, PlanModifiers } from "@/lib/checkin/types";
...
export const derivePlanModifiers = (checkin: NormalizedDailyCheckin): PlanModifiers => {
  const modifiers: PlanModifiers = {
    ...
    exercise_constraint: null,
    add_micro_actions: [],
    reasoning_short: "Today looks like a steady, supportive plan.",
  };

  const poorSleep = checkin.sleep_quality_score === 1;
  const highStress = checkin.stress_level >= 4;
  const lowEnergy = checkin.energy_level <= 2;
  const hasInjury = checkin.context_tags.includes("injury");
  ...
  if (highStress) {
    modifiers.focus = "stress_support";
    modifiers.add_micro_actions = Array.from(new Set([...modifiers.add_micro_actions, "breathwork_5min"]));
  }
  ...
  if (modifiers.focus === "recovery") {
    modifiers.reasoning_short = "We’ll keep today gentle and recovery-focused.";
  } else if (modifiers.focus === "stress_support") {
    modifiers.reasoning_short = "We’ll keep things calm and supportive today.";
  } else if (lowEnergy) {
    modifiers.reasoning_short = "We’ll keep today light and time-friendly.";
  }
