...
export interface ExercisePreferences {
  path: 'smart-match' | 'build-your-own';
  // Smart Match data
...
interface ExerciseSetupWizardProps {
  isOpen: boolean;
  onClose: () => void;
  onComplete: (program: ExerciseProgram, preferences: ExercisePreferences) => void;
}

type WizardStep = 'path-selection' | 'smart-match' | 'build-your-own' | 'program-selection' | 'confirmation';

export const ExerciseSetupWizard = ({ isOpen, onClose, onComplete }: ExerciseSetupWizardProps) => {
  ...
  const handleConfirm = () => {
    if (selectedProgram) {
      onComplete(selectedProgram, preferences);
    }
  };
  ...
  const handleClose = () => {
    setCurrentStep('path-selection');
    setPreferences({ path: 'smart-match' });
...
export default ExerciseSetupWizard;
