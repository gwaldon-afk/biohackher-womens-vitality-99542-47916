      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          <h2 className="text-2xl font-semibold text-center mb-6">
            {user && assessedSymptoms.length > 0 ? "Available Symptoms Assessment" : "Choose Your Symptom Assessment"}
          </h2>
                  ...
                  <label
                    ...
                    className={`cursor-pointer flex flex-col items-center p-4 rounded-lg border-2 transition-all hover:scale-105 hover:shadow-md w-full max-w-[140px] ${
                      selectedSymptom === symptom.id
                        ...
                        : 'border-secondary/30 bg-background hover:border-secondary/50 hover:bg-secondary/5'
                    }`}
                    onClick={() => handleSymptomSelect(symptom.id)}
                  >
                    <symptom.icon className={`h-8 w-8 mb-3 ${
                      selectedSymptom === symptom.id ? 'text-secondary-foreground' : 'text-secondary'
                    }`} />
                    <span className={`text-sm font-medium text-center leading-tight mb-2 ${
                      selectedSymptom === symptom.id ? 'text-secondary-foreground' : 'text-foreground'
                    }`}>
                      {symptom.name}
                    </span>
                    <Badge variant={selectedSymptom === symptom.id ? "secondary" : "outline"} className="text-xs">
                      Assess
                    </Badge>
                  </label>
                </div>
