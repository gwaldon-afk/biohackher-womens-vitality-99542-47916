  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        {/* Symptom Selection - Horizontal Radio Buttons */}
        <div className="mb-8">
          <h2 className="text-2xl font-semibold text-center mb-6">Choose Your Symptom</h2>
                  ...
                  <label
                    ...
                    className={`cursor-pointer flex flex-col items-center p-4 rounded-lg border-2 transition-all hover:scale-105 hover:shadow-md w-full max-w-[140px] ${
                      selectedSymptom === symptom.id
                        ...
                        : 'border-gray-200 bg-primary hover:border-primary/50 hover:bg-primary-light'
                    } ${isActive ? 'ring-2 ring-green-500/20' : ''}`}
                    onClick={() => handleSymptomSelect(symptom.id)}
                  >
                    <symptom.icon className={`h-8 w-8 mb-3 ${
                      selectedSymptom === symptom.id ? 'text-primary' : 'text-gray-600'
                    }`} />
          ...
          {selectedSymptom && (
            <div className="text-center mt-6">
              <Button
                onClick={() => handleSymptomSelect(selectedSymptom)}
                className="px-8 py-3 text-lg"
              >
                Start Assessment for {symptoms.find(s => s.id === selectedSymptom)?.name}
              </Button>
            </div>
          )}
        </div>
