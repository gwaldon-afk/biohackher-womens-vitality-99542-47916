...
import { Thermometer, Moon, Brain, UtensilsCrossed, FileText, Printer, Bone, Zap, Battery, Wind, Scale, Scissors, Heart, Calendar, Headphones, Droplets, BookOpen, Settings, Plus } from "lucide-react";
...
const Symptoms = () => {
  ...
  // Filter symptoms based on user selections
  const displayedSymptoms = symptoms.filter(symptom => {
    if (!user || userSymptoms.length === 0) return true; // Show all if not logged in or no preferences
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-muted-foreground">
                Commonly used complementary treatments for symptoms during hormonal transitions
              </p>
            </div>
            {user && (
              <Button
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-xl font-semibold">
                {user && !isCustomizing ? "Your Symptoms" : "Symptoms"}
              </h2>
              {user && displayedSymptoms.length === 0 && !isCustomizing && (
                <Button
                  variant="outline"
                  size="sm"
            ...
            {user && displayedSymptoms.length === 0 && !isCustomizing ? (
              <Card className="p-6 text-center">
                <p className="text-muted-foreground mb-4">No symptoms selected yet</p>
                <Button
                  onClick={() => setIsCustomizing(true)}
                  className="flex items-center gap-2"
                >
              ...
              <>
                {(user ? displayedSymptoms : symptoms).map((symptom) => {
                  const userSymptom = getUserSymptomData(symptom.id);
                  return (
                    <Card
                      key={symptom.id}
                          ...
                          <div>
                            ...
                            <div className="flex gap-2 mt-1">
                              <Badge variant="secondary" className="text-xs">
                                {userSymptom?.severity || symptom.severity}
                              </Badge>
                              <Badge variant="outline" className="text-xs">
                                {userSymptom?.frequency || symptom.frequency}
                              </Badge>
                            </div>
                            {userSymptom?.notes && (
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {user && (
                      <Badge variant="outline" className="ml-2">
                        {getUserSymptomData(currentSymptom.id)?.severity || currentSymptom.severity}
                      </Badge>
                    )}
                  </div>
                  <Button
                    ...
                    <Card key={index} className="card-elevated">
                      ...
                      <CardContent>
                        <Tabs defaultValue="protocol" className="w-full">
                          <TabsList>
                            <TabsTrigger value="protocol">Information</TabsTrigger>
                            <TabsTrigger value="contraindications">Safety</TabsTrigger>
                          </TabsList>
                          <TabsContent value="protocol" className="mt-4">
                            <div className="space-y-2">
                              <p className="text-sm text-muted-foreground">
                                <strong>Commonly used approaches:</strong>
                              </p>
                          ...
                          <TabsContent value="contraindications" className="mt-4">
                            <div className="space-y-2">
                              ...
                              <p className="text-xs text-muted-foreground mt-2">
                                <strong>Disclaimer:</strong> This is educational information only, not medical advice. 
                                Always consult with your healthcare provider before starting new interventions. 
                                Individual responses vary and professional guidance is essential for safety.
                              </p>
                            </div>
                          </TabsContent>
                        </Tabs>
              ...
              <div className="flex items-center justify-center h-64">
                <div className="text-center">
                  <p className="text-muted-foreground mb-4">
                    {user && displayedSymptoms.length === 0 
                      ? "Add symptoms to see treatment recommendations" 
                      : "Select a symptom to view treatment information"
                    }
                  </p>
                  {user && displayedSymptoms.length === 0 && (
                    <Button
