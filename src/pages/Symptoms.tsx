...
const Symptoms = () => {
  ...
  // Triage flow state
  const [triageStep, setTriageStep] = useState<'entry' | 'pillar-overview' | 'selection' | 'browse-all' | 'dashboard'>('entry');
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        {triageStep === 'entry' && (
          <SymptomTriageEntry
            onThemeSelect={(themeId) => {
              ...
              setTriageStep('pillar-overview');
            }}
            onBrowseAll={() => setTriageStep('browse-all')}
          />
        )}

        {triageStep === 'pillar-overview' && selectedTheme && (
          <PillarOverviewCard
            pillarId={selectedTheme}
            symptoms={symptoms}
            userSymptoms={userSymptoms}
            onBack={() => setTriageStep('entry')}
            onStartAssessment={handleSymptomSelect}
            onBrowseAll={() => setTriageStep('browse-all')}
          />
        )}
        ...
        {triageStep === 'selection' && selectedTheme && (
          <SymptomAssessmentSelection
            ...
            symptoms={symptoms}
            userSymptoms={userSymptoms}
            onBack={() => setTriageStep('pillar-overview')}
            onBrowseAll={() => setTriageStep('browse-all')}
          />
        )}
